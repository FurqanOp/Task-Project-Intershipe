...
# Gemini API Setup
API_KEY = "AIzaSyBOy-VukVYWTQJA8J6sI6_Xu4hmCsORQfo"
...
def summarize_with_gemini(text):
    try:
        ...
        st.write("Prompt sent to Gemini (first 500 chars):")
        st.write(prompt[:500])
        ...
        st.write("Full response object from Gemini:")
        st.write(response)

        generated_text = response.candidates[0].output if response.candidates else ""
                        ...
                        if st.button("Summarize with Gemini"):
                            with st.spinner("Summarizing transcript with Gemini..."):
                                ...
                                if summary:
                                    ...
                                    st.info("No summary returned from Gemini.")
                    else:
